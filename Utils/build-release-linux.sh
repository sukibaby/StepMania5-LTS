...
docker build --platform=${ARCH} -f Dockerfile-linux . -t SM5-LTS-linux-build:${ARCH}

docker run -i -v $(pwd)/..:/data:Z --platform=linux/${ARCH} SM5-LTS-linux-build:${ARCH} sh -eux <<'EOF'
...
cp /tmp/Build/SM5-LTS-* /data/Build/release/
EOF
